    ...
    display() {
        ...
        this.buttons.forEach((button, index) => {
            if (index === this.selectedButtonIndex) {
                this.context.fillText("BBBBBBB " + button.label, 50, yOffset);
                this.renderImage(button.selectedImage ? button.selectedImage : button.image, 200, yOffset - 20);
            } else {
                this.context.fillText("  AAAAAAAA " + button.label, 50, yOffset);
...
// Exemplo de uso
canvas = document.getElementById("#canvasCL");
context = canvas.getContext("2d");
const menu = new Iniciarmenu(canvas, context);
menu.display();
